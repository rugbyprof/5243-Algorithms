...
// ------------------------------
class BST final : public Structure {
private:
    struct Node {
        int data;
        Node *left;
    ...
    // Count exactly ONE key-compare per node visit by using a 3-way compare.
    ...
    void insert_impl(Node *&n, int x) {
        if (!n) {
            // Inserting a brand new node: treat as a structural change.
            relink();
            n = new Node(x);
            size_++;
            return;
        ...
        int r = cmp_key(x, n->data);
        if (r < 0)
            insert_impl(n->left, x);
        else
            insert_impl(n->right, x); // duplicates go right
    }
